...
public class ActionManager : MonoBehaviour
{
    ...
    private void ShowCreateUnitButtons(Building b)
    {
        if (b.IsFunctional)
        {
            for (int i = 0; i < b.UnitPrefabs.Length; i++)
            {
                ...
                Units unit = b.UnitPrefabs[i].GetComponent<Units>();
                unitBtns[i].image.sprite = unit.UnitPic;
            }
        }
    }
    private void ShowCreateBuildingButtons(Units u) //Showing list of buildings when selecting a single unit
    {
        if (u.IsBuilder)
        {
    ...
    public void ShowBuilderMode(Units unit)
    {
        ClearAllInfo();
    ...
    public void CreateBuildingButton(int n)//Map with Create Building Btns
    {
        ...
        Units unit = UnitSelect.instance.CurUnit[0];
